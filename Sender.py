...
class Sender(BasicSender.BasicSender):
    def __init__(self, dest, port, filename, debug=False, sackMode=False):
        ...
        self.debug = debug
        self.initial_seqnum = randint(3578,5437)# generates inital sequence number
        self.running_index = 0                  # stores base index
        self.recv_buffer = []                   # stores raw data from receiver
        self.msg_buffer = []                    # hold message read from file
        self.seqno_buffer = []                  # holds all sequence number to associate with data
        self.list_of_expected_seqno = []        # holds list of expected serial to be returned from receiver after transmission
        self.validated_packts = []              # holds already validated serial numbers
    ...
    def start(self):
        ...
        # reset variables used in handshake
        ...
        while self.msg_buffer[self.running_index:] != []:
            ...
            if self.recv_buffer is None or self.recv_buffer == []:
                continue
    ...
    def fast_retransmit(self, packets):
        ...
        for x in set(packets):
            ...
            if packets.count(x) >= 4:
                ...
                self.recv_buffer = filter(None, tmp_)
                if self.recv_buffer is None or tmp_ == []:
                    continue
